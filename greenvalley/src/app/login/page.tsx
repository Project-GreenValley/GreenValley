...
export default function Login() {
    ...
    // FUTURE WORK --- useEffect here for checking if user is already authenticated (e.g., has a valid token or session), or look into React hooks (useUser, useContext)

    // write handle login submit function here
    const handleLoginSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();

        // grab username and password (FormData, part of Javascript API --- extracts form inputs using the name attribute)
    ...
    const handleSignupSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // grab new username and new password
        const signupForm= e.currentTarget;
        const signupFormData = new FormData(signupForm);
        const newUsername = signupFormData.get('username') as string;
        const newPassword = signupFormData.get('password') as string;
    ...
    return(
        <div className='flex flex-col items-center justify-center'>
            ...
            {/* dividing line between login and sign up sections */}
